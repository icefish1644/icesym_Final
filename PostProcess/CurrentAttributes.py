...
class ICESymMainWindow(QtWidgets.QMainWindow):
    def __init__(self, case_name = None, case_dir = None, case_type = None):
        ...
        if case_type in (1,3): # case opened or created with wizard (previously saved)
            self.open_data()
        elif case_type==2: # new case blank
    ...
    def modify_conections_items(self, item1, item2, new_connection=False):
        """
        Set the necessary arguments for each item according to the connection
        item1: item to the left of the connection
        item2: item to the right of the connection
        """
        item1_index = self.objects[item1.type].index(item1)
        item2_index = self.objects[item2.type].index(item2)
        
        if item1.type=='Tubes':
            # Special valve case, you have to put the cylinder
